...
echo [1] Quick build starting...
echo Work path: %WORK_DIR%
echo Dist path: %DIST_DIR%

if defined PYTHON_CMD (
    "%PYTHON_CMD%" -m PyInstaller --noconfirm --workpath "%WORK_DIR%" --distpath "%DIST_DIR%" app_onedir.spec
) else (
    set "UV_CACHE_DIR=%CD%\.uv-cache"
    uv run --python 3.14 --with pyinstaller --with streamlit --with pandas --with requests --with beautifulsoup4 --with lxml python -m PyInstaller --noconfirm --workpath "%WORK_DIR%" --distpath "%DIST_DIR%" app_onedir.spec
)
